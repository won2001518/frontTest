...
function regExCheck() {
  ...
  const regEx8 = /[^a-zA-Z0-9]/g;  // 영문 대/소문자+숫자만포함?
  const regEx9 = /[^0-9가-힣]/g;  // 한글+숫자만포함?
  const regEx10 = /[^a-zA-Z0-9_-]/g;  // 영문 대/소문자+'_'+'-'숫자만포함?
  ...
  // if(regEx9.test(content)) alert("본문에 숫자,한글이 아닌 문자가 있습니다.");
  // if(regEx10.test(content)) alert("본문에 영문 대/소문자/숫자/_/-가 아닌 문자가 있습니다.");
  // if(!regEx21.test(content)) alert("본문에 '.'이 아닌 문자가 있습니다.");
  // if(!regEx22.test(content)) alert("본문에 숫자가   아닌 문자가 있습니다.");
  // if(regEx23.test(content)) alert("본문에 숫자가 아닌값이 있습니다.");
  // if(!regEx24.test(content)) alert("본문에 영문/숫자/_가 아닌값이 있습니다.");
